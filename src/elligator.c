...
CurvePoint elligator_1_string_to_point(BigInt *t, Curve curve)
{
    ...
    C2 = big_int_inverse(C2, C2, curve.q);
    ...
    BigInt *X_plus_1 = big_int_add(NULL, big_int_one, X, curve.q);
    ...
    BigInt *x1 = big_int_inverse(NULL, Y, curve.q);
    BigInt *x = big_int_mul(NULL, x0, x1); // x = (c − 1)*s*X*(1 + X) / Y

    BigInt *rX = big_int_mul_mod(NULL, curve.r, X, curve.q);
    BigInt *y0 = big_int_sub(NULL, rX, X_plus_1_squared, curve.q);

    BigInt *y1 = big_int_add(NULL, rX, X_plus_1_squared);
    y1 = big_int_inverse(y1, y1, curve.q);

    BigInt *y = big_int_mul(NULL, y0, y1, curve.q); //  y = (rX − (1 + X)**2) / (rX + (1 + X)**2)
...
BigInt elligator_1_point_to_string(CurvePoint p, Curve curve)
{
    ...
    X0 = big_int_inverse(X0, big_int_four, X0);
    q_1 = big_int_mul_mod(q_1, q_1, X0, curve.q);

    BigInt *X1 = big_int_pow(NULL, eta_r, create_big_int(2), curve.q);
    ...
    BigInt *x3 = big_int_inverse(NULL, curve.r, curve.q);
    ...
    z3 = big_int_inverse(z3, z3, curve.q);
    ...
    t1 = big_int_inverse(t1, t1, curve.q);
