...
BigInt *big_int_add_256(BigInt *r, BigInt *a, BigInt *b)
{
    ...
    ADD_STAT_COLLECTION(BASIC_ADD_CHUNK)
    ADD_STAT_COLLECTION(BASIC_ADD_CHUNK) ADD_STAT_COLLECTION(BASIC_SHIFT)
    ...
    for (i = 0; i < a->size; ++i)
    {
        ...
        else
        {
            ...
            for (j = i + 1; j < a->size; j++)
            {
                ...
                ADD_STAT_COLLECTION(BASIC_ADD_CHUNK)
                ADD_STAT_COLLECTION(BASIC_ADD_CHUNK) ADD_STAT_COLLECTION(BASIC_ADD_OTHER)
...
BigInt *big_int_div_rem(BigInt *q, BigInt *r, BigInt *a, BigInt *b)
{
    ...
    else
    {
        ...
        for (q_idx = q->size - 1; q_idx >= 0; --q_idx)
        {
            ...
            {
                ADD_STAT_COLLECTION(BASIC_ADD_CHUNK)
                ADD_STAT_COLLECTION(BASIC_ADD_OTHER) ADD_STAT_COLLECTION(BASIC_ADD_OTHER) ADD_STAT_COLLECTION(BASIC_MUL_CHUNK) ADD_STAT_COLLECTION(BASIC_MUL_CHUNK) if (q_c >= BIGINT_RADIX || (b->size > 1 && q_c * b_loc->chunks[b->size - 2] >
...
BigInt *big_int_srl_small(BigInt *r, BigInt *a, uint64_t shift)
{
    ...
    {
        ADD_STAT_COLLECTION(BASIC_ADD_OTHER)
        ADD_STAT_COLLECTION(BASIC_ADD_OTHER) ADD_STAT_COLLECTION(BASIC_ADD_OTHER)
            ...
            if (a->sign && (shift - 1) / BIGINT_CHUNK_BIT_SIZE < a->size)
        {
            ...
            big_int_create_from_chunk(r,
                                      ...
                                      1);
            ADD_STAT_COLLECTION(BASIC_SHIFT)
            ADD_STAT_COLLECTION(BASIC_MOD) ADD_STAT_COLLECTION(BASIC_BITWISE)
        }
        else
        {
